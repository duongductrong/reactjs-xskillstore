...
function isProductCorrect(products) {
	...
	try {
		for (let i = 0; i < products.length; i++) {
			...
			if (
				Checking.isNull(products[i].id_product) ||
...
module.exports.checkGetParchaseds = async (req, res, next) => {
	let { page = 1, onpage = 9, verify, cancel, sort = 'desc' } = req.query;
	let condition = {};
...
module.exports.checkUserOrders = async (req, res, next) => {
	...
	let user_orders = await PURCHASED_MODEL.find({ user: { $in: userLogin._id } })
		.skip(pagination.start)
		.limit(pagination.end)
		.sort({ _id: 'DESC' });

	let spent = await PURCHASED_MODEL.aggregate([
		{
...
module.exports.checkCreate = async (req, res, next) => {
	...
	let errors = {};
	let { products, user, verify, message, cancel, price, promotion } = req.body;
	let total_money = 0;
	...
	if (Checking.isNull(products) || products.length < 0) {
		errors.products = Notification.message(
			'Sản phẩm không được để trống, khi mua phải bao gồm sản phẩm được mua',
			'error',
			404
		);
	} else {
		if (!isProductCorrect(products)) {
			errors.products = Notification.message(
				'Biểu mẫu sản phẩm gửi lên không chính xác, vui lòng không chỉnh sửa thông tin. Đảm bảo số lượng, giá tiền không nhỏ hơn 0',
				'error',
				404
			);
		} else {
			...
			if (!checkProduct.exists) {
				errors.products = Notification.message(
					'Một trong số các mã hàng hóa này hiện không còn tồn tại, vui lòng thử lại',
					'error',
					400
				);
			}
