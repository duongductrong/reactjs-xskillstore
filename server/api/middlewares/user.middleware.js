...
module.exports.checkGetUser = async (req, res, next) => {
	...
	try {
		...
		res.json(Notification.message('Không tìm thấy ID người dùng này.', 'error', 404, { user: {} }));
		return;
	}
};
...
module.exports.checkCreate = async (req, res, next) => {
	...
	let hash;
	let { username, password, firstname, lastname, address, email, number_phone, permission } = req.body;
	...
	if (Checking.isNull(password)) {
		...
		if (!Checking.isPassword(password)) {
			errors.password = Notification.message(
				'Mật khẩu không hợp lệ, phải có đầy đủ ký tự hoa, thường, số',
				'error',
				400
			);
		} else {
...
module.exports.checkUpdate = async (req, res, next) => {
	...
	if (userLogin.permission === 'admin') {
		...
		if (Checking.isNull(permission)) {
			...
			if (
				!Checking.compare('admin', permission) &&
				!Checking.compare('member', permission) &&
				!Checking.compare('tester', permission)
			) {
...
module.exports.checkAuth = async (req, res, next) => {
	...
	if (!header) {
		res.json(
			Notification.message('Yêu cầu xác thực thất bại, không nhận được mã xác thực, vui lòng đăng nhập', 'error', 403)
		);
		return;
	}
