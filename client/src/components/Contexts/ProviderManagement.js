...
class ProviderManagement extends React.Component {
			...
			Axios({
				...
				.then((res) => {
					...
					if (data.status === 'error') {
						this.props.toast.error(data.msgVi, { closeTimeout: 3000, position: 'bottom-right' });
					} else {
						this.DeleteAllCart();
						this.props.toast.success('Gửi đơn hàng thành công', { closeTimeout: 3000, position: 'bottom-right' });
					}
				})
				.catch((err) => {
					window.alert(err);
	...
	render() {
		...
		statePayments = stateCart
			? stateCart.reduce((sum, item) => {
					return sum + item.price * item.amountSelected;
			  }, 0)
			: 0; // tính tiền //Counter money
		return (
			<Context.Provider
				value={{
...
export default withToast(ProviderManagement);
