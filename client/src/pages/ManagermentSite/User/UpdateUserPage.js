...
import authConfig from '../../ManagermentSite/config/config';
		...
		return (
			<Management>
				...
				<Box title='Thông tin cá nhân người dùng'>
					<Alert danger>
						<b>Cảnh báo</b>: Đây là thông tin người dùng, chỉnh sửa sẽ vi phạm quyền riêng tư của người dùng
					</Alert>
					<form onSubmit={this.onSubmit} style={{ marginTop: '15px' }} className='update-user-page'>
							...
							<Group>
								<label htmlFor='firstname'>
									...
									{this.state.errors && this.state.errors.firstname && (
										<ErrorText> {this.state.errors.firstname.msgVi} </ErrorText>
									)}{' '}
								</label>
								<Input
									onChange={this.onFirstnameStateChange}
									value={firstname}
									id='firstname'
									type='text'
							...
							<Group>
								<label htmlFor='lastname'>
									...
									{this.state.errors && this.state.errors.lastname && (
										<ErrorText> {this.state.errors.lastname.msgVi} </ErrorText>
									)}{' '}
								</label>
								<Input
									onChange={this.onLastnameStateChange}
									value={lastname}
									id='lastname'
									type='text'
							...
							<Group>
								<label htmlFor='email'>
									...
									{this.state.errors && this.state.errors.email && (
										<ErrorText> {this.state.errors.email.msgVi} </ErrorText>
									)}{' '}
								</label>
								<Input
									onChange={this.onEmailStateChange}
									value={email}
									id='email'
									type='email'
									placeholder='Email'
						...
						<label htmlFor='username'>
							...
							{this.state.errors && this.state.errors.username && (
								<ErrorText> {this.state.errors.username.msgVi} </ErrorText>
							)}{' '}
						</label>
						<Input
						...
						<label htmlFor='address'>
							...
							{this.state.errors && this.state.errors.address && (
								<ErrorText> {this.state.errors.address.msgVi} </ErrorText>
							)}{' '}
						</label>
						<Input
						...
						<label htmlFor='numberPhone'>
							...
							{this.state.errors && this.state.errors.number_phone && (
								<ErrorText> {this.state.errors.number_phone.msgVi} </ErrorText>
							)}{' '}
						</label>
						<Input
							onChange={this.onNumberPhoneStateChange}
							value={number_phone}
							id='numberPhone'
							type='text'
						...
						<label htmlFor='permission'>
							<small>Quyền</small>{' '}
							{this.state.errors && this.state.errors.permission && (
								<ErrorText> {this.state.errors.permission.msgVi} </ErrorText>
							)}{' '}
						</label>
						<Input
							onChange={this.onPermissionStateChange}
							value={permission}
							id='permission'
							type='text'
							placeholder='Phân cấp quyền'
							square
						/>
