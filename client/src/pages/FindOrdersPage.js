...
import { withHttpRequest } from '../hooks/useHttpRequest';
			...
			this.props.http
				...
				.then((res) => {
					...
					if (data.status === 'error') {
						this.props.toast.error(data.msgVi || 'Không tìm thấy đơn hàng này', {
			...
			<Store>
				<LayoutMedium className='find-orders-page'>
					<div className='find-orders-page__box'>
						...
						<InputGroup>
							<Input
								onChange={this.onCodeOrdersStateChange}
								value={codeOrders}
								width='100%'
								type='text'
								...
								square
							/>
							<Button onClick={this.onFindOrders(codeOrders)} style={{ width: '100%' }} type='submit'>
								Tìm kiếm ngay
							</Button>
						</InputGroup>
						<div className='find-orders-page__box__status'>
							<div>
								...
								<Process
									ratio={
										purchased && purchased !== null
											? !purchased.verify && !purchased.cancel
												? 50
												: purchased.verify && !purchased.cancel
												? 100
												: !purchased.verify && purchased.cancel
									...
									margin='5px 0'
									danger={
										purchased && purchased !== null ? (!purchased.verify && purchased.cancel ? true : false) : false
									}
									success={
										purchased && purchased !== null ? (purchased.verify && !purchased.cancel ? true : false) : false
									}
									...
									<small>
										{purchased && purchased !== null
											? !purchased.verify && !purchased.cancel
												...
												: !purchased.verify && purchased.cancel
												? 'Đơn hàng bị từ chối, sẽ được liên hệ lý do'
												: 'Chưa xác nhận'
								...
								<h5>
									...
									{purchased && purchased !== null
										? purchased.products.map((item, i) => (
												<NavLink target='_blank' to={`/product/${item.id_product}`} key={`${i}-findOrdersPage-product`}>
													{' '}
													{item.id_product}{' '}
												</NavLink>
										  ))
								...
								<h5>
									c. Tổng giá: {purchased && purchased !== null ? Module.AddCommaToNumber(purchased.price) : '0'} VND{' '}
								</h5>
								<h5>
									d. Khuyến mãi:{' '}
									{purchased && purchased !== null
										? purchased.promotion !== ''
											? JSON.parse(purchased.promotion).name
											: ''
										: 'Không có'}{' '}
								</h5>
							</div>
...
export default withToast(withHttpRequest(FindOrdersPage));
