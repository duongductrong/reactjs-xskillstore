...
class ProductListPage extends React.Component {
	...
	onPagination(page, onPage) {
		return () => {
			...
			query += `&gender=${
				filter.gender.length > 1 ? filter.gender.join(',') : filter.gender.length !== 0 ? filter.gender[0] : ''
			}`;
			query += `&size=${
				filter.size.length > 1 ? filter.size.join(',') : filter.size.length !== 0 ? filter.size[0] : ''
			}`;
			query += `&status=${
				filter.status.length > 1 ? filter.status.join(',') : filter.status.length !== 0 ? filter.status[0] : ''
			}`;
			...
			this.setState(
				{
					gender:
						id === 1
							? { all: true, male: false, female: false }
							: id === 2
							? { all: false, male: true, female: false }
							: id === 3
							? { all: false, male: false, female: true }
							: { all: true, male: false, female: false },
				},
				callback
			);
	...
	onFilters(min, max, gender, size, status, special) {
		return (event) => {
			...
			if (special === 'size') {
				if (size) {
					conditions.size =
						[...filter.size].indexOf(Number(size)) === -1 //Conditions add or move
			...
			if (special === 'status') {
				if (status === 0 || status === 1) {
					conditions.status =
						[...filter.status].indexOf(Number(status)) === -1
				...
				<LayoutMedium className='product-list'>
					<div className='product-list__side'>
						<div className='product-list__side__option'>
							<TextOption
								filter={filter}
								onClick={this.onActivedGenderChange(
									1,
									this.onFilters(filter.min_price, filter.max_price, [0, 1], filter.size, filter.status, 'gender')
								)}
								actived={this.state.gender.all}
							>
								Tất cả
							</TextOption>
							<TextOption
								...
								onClick={this.onActivedGenderChange(
									2,
									this.onFilters(filter.min_price, filter.max_price, 0, filter.size, filter.status, 'gender')
								)}
								actived={this.state.gender.male}
							>
								Nam
							</TextOption>
							<TextOption
								...
								onClick={this.onActivedGenderChange(
									3,
									this.onFilters(filter.min_price, filter.max_price, 1, filter.size, filter.status, 'gender')
								)}
								actived={this.state.gender.female}
							>
								Nữ
							</TextOption>
						</div>
						<div className='product-list--separate'></div>
						<CollapseText title='Trạng thái'>
							{this.collapse_data.status.map((item, i) => (
								<CollapseTextElement
									...
									active={
										filter.status.find((e) => e === item.id) === 0 || filter.status.find((e) => e === item.id) === 1
											? true
											: false
									}
								>
						...
						<div className='product-list--separate'></div>
						<CollapseText title='kích thước' inline>
						...
						<CollapseText title='giá'>
							<InputGroup group>
								<Group>
									<label>
										<small>Từ: </small> {Module.AddCommaToNumber(filter.min_price)}{' '}
									</label>
									<Input
										onChange={this.onMinPriceStateChange}
										type='range'
										min='0'
										max='20000000'
								...
								<Group>
									<label>
										<small>Đến: </small> {Module.AddCommaToNumber(filter.max_price)}{' '}
									</label>
									<Input
										onChange={this.onMaxPriceStateChange}
										type='range'
										min='50000000'
										max='90000000'
							...
							<Button type='button' onClick={this.onApplyPriceToFilter} style={{ display: 'block', width: '100%' }}>
								Áp dụng
							</Button>
						</CollapseText>
					</div>
					<div className='product-list__main'>
						<img src={PosterImg} className='product-list__main__image' />
						<div className='product-list__main__products'>
							{products.length > 0 ? (
								...
								<Alert style={{ gridColumn: '1 / span 3', textAlign: 'center' }} danger>
									{' '}
