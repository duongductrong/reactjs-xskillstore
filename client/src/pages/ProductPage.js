					...
					if (data.status === 'error') {
						...
						this.setState(
							...
							() => {
								...
								document.getElementById('information_product-js').innerHTML = product.information_production;
							}
						);
					}
				})
					...
					{({ FuncAddToCart }) => (
						<LayoutMedium className='product-page'>
							<div className='product-page__images'>
								<PortfolioSlideImage
									images={product.images_detail}
									image={product.images_detail && product.images_detail[0]}
								/>
							</div>
							<div className='product-page__detail'>
								...
								<p className='product-page__detail__species'>
									<span className='product-page__detail__species__code'>
										Mã sản phẩm: <b> {product._id} </b>
									</span>
									<span className='product-page__detail__species__status'>
										Tình trạng: <b> {product.status ? 'Còn hàng' : 'Hết hàng'} </b>
									</span>
								</p>
								...
								<InputGroup group>
									<Group>
										<label>
											<small>Số lượng</small> {errors.amount && <ErrorText> Không được vượt quá giới hạn </ErrorText>}{' '}
										</label>
										<Input
											onChange={this.onAmountStateChange}
											type='number'
											min='1'
											max='15'
									...
									<Group>
										<label>
											<small>Size</small> {errors.size && <ErrorText> Không được vượt quá giới hạn </ErrorText>}{' '}
										</label>
										...
										<select
											...
											style={{ width: '100%', padding: '9px', borderRadius: '5px', margin: '10px 0' }}
										>
											{product.size &&
								...
								<div className='product-page__detail__btn'>
									<Button
										onClick={FuncAddToCart({ ...product, sizeSelected: size, amountSelected: amount })}
										style={{ padding: '20px', width: '70%', marginRight: '5%' }}
									>
										Thêm vào giỏ hàng
									</Button>
								...
								<Seperate dashed />
								<CollapseText title='Bảo hành sản phẩm'>{product.guarantee}</CollapseText>
								<Seperate dashed />
							</div>
						</LayoutMedium>
					)}
